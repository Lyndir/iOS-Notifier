...
public interface APNClientService {
     ...
     * 
     * @param callback
     *            The instance to notify when the unreachable devices have been determined. Use <code>null</code> if
     *            you're not interested in feedback but just want to clear the Apple Push Notification Feedback
     *            Service's data queue.
     * 
     * @throws IOException
     *             If the system failed to initiate a connection to the Apple Feedback service.
     ...
     * 
     * <p>
     * You should probably use {@link #queueNotification(NotificationDevice, JSONObject, NotificationPayLoad...)}
     * instead.
     * </p>
     * 
     * @param device
     ...
     *            modest. You can specify <code>null</code> here to omit any custom data.
     * @param notificationPayLoads
     *            The JSON representation of the notification payloads. These payloads describe the actual events the
     *            notification should trigger on the device. You can specify multiple notification events, and must
